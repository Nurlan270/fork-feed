<div class="flex flex-nowrap md:justify-end justify-between sm:justify-end items-center md:gap-x-2 sm:gap-x-2">
    <!-- Like Button -->
    <button title="{{ __('livewire_components.like') }}" wire:click="{{ $liked ? 'resetReaction' : 'like' }}"
            type="button"
    ...
    <!-- Dislike Button -->
    <button title="{{ __('livewire_components.dislike') }}" wire:click="{{ $disliked ? 'resetReaction' : 'dislike' }}"
            type="button"
    ...
    <!-- Bookmark Button -->
    <button title="{{ __('livewire_components.bookmark') }}" type="button"
            wire:click="{{ $bookmarked ? 'removeBookmark' : 'bookmark' }}"
            class="flex items-center px-3 py-2 rounded-md text-primary-700 hover:bg-gray-100 transition cursor-pointer text-sm sm:text-base">
        @if($bookmarked)
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-5">
