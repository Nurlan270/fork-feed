        ...
        <div class="space-y-3 flex-1">
            ...
            <div class="flex items-start justify-between gap-3">
                    ...
                    @canany(['update', 'delete'], $recipe)
                        <div class="flex items-center gap-1 flex-shrink-0">
                            ...
                            <button
                                ...
                                title="{{ __('recipe/card.actions.delete') }}"
                                wire:click="deleteRecipe({{ $recipe->id }})"
                                wire:confirm="{{ __('recipe/card.actions.delete_confirm') }}">
                ...
                @elseif($showRemoveBookmarkButton && $isBookmarked)
                    <div class="flex items-center gap-1 flex-shrink-0">
                        <button type="submit"
                                class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors cursor-pointer"
                                title="{{ __('recipe/card.actions.remove_bookmark') }}"
            ...
            <div class="flex flex-wrap items-center text-xs md:text-sm text-gray-600 gap-3 md:gap-4 mb-4">
                ...
                <a class="flex items-center whitespace-nowrap hover:underline truncate max-w-32 md:max-w-none"
                   href="{{ getLocalizedURL('user.tag-profile', ['user' => $recipe->author->username]) }}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                ...
                {{-- Views --}}
                <span class="flex items-center gap-x-1 whitespace-nowrap" title="{{ __('recipe/card.meta.views') }}">
                ...
                {{-- Date --}}
                <span class="flex items-center gap-x-1 whitespace-nowrap" title="{{ __('recipe/card.meta.date') }}">
                       <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                            class="size-4 text-gray-500 flex-shrink-0">
                          <path
                        ...
                        {{ Carbon::parse($recipe->created_at)->translatedFormat('d M Y') }}
                    </span>
            </div>
