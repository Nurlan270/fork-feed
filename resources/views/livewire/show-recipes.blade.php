<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h2 class="text-3xl font-semibold">{{ __('recipes.title') }}</h2>
        ...
        <div class="flex w-full max-w-lg">
            ...
            >
                <option selected disabled>{{ __('recipes.sort_by.label') }}</option>
                <option value="popular">{{ __('recipes.sort_by.popular') }}</option>
                <option value="liked">{{ __('recipes.sort_by.liked') }}</option>
                <option value="newest">{{ __('recipes.sort_by.newest') }}</option>
                <option value="oldest">{{ __('recipes.sort_by.oldest') }}</option>
            </select>
        </div>
    </div>
    ...
    @if($recipes->isNotEmpty())
        ...
        {{ $recipes->links() }}
    @else
        <p class="text-base text-center mt-10 break-words">{{ __('recipes.nothing_found') }}</p>
    @endif
</div>
